...
class CarbonFootprintCalculator(QMainWindow):
    ...
    def check_employee_count(self):
        try:
            ...
            if staff_count == 1:
                ...
                self.fetch_europe_avg_and_update_table("Small Enterprises", year)
                ...
                self.fetch_europe_avg_and_update_table("Large Enterprises", year)
    ...
    def generate_feedback(self):
        try:
            ...
            industry_avg_cf = self.carbonCalculator["Details"].get("Industry_Avg_CO2", 0)

            # Determine which average to use for comparison
            if self.sbusiness_rbtn.isChecked() or self.bbusiness_rbtn.isChecked():
                # Compare with Industry Average CF for businesses
                chosen_avg = industry_avg_cf
                avg_type = "Industry Average CF"
            else:
                # Compare with Europe Average CF for individuals
                chosen_avg = avg_cf
                avg_type = "Europe Average CF"

            # Generate feedback based on the comparison
            if percapita_AvgCF < chosen_avg:
                # Positive Feedback
                feedback_text = (
                    f"<b>Good/Positive Result:</b><br>"
                    f"Fantastic work! Your carbon footprint is below the {avg_type}.<br>"
                    f"Keep up these sustainable habits, and consider additional ways to further reduce your impact.<br><br>"
                    f"<b>Energy:</b> You're doing great! Consider adding renewable energy options, like solar panels, to take it a step further.<br>"
                    f"<b>Waste:</b> Keep up the progress! Composting organic waste and promoting recycling can make an even bigger difference.<br>"
                    f"<b>Travel:</b> Excellent choices! Continue carpooling or using public transport whenever possible to maintain a low travel footprint."
                )
            else:
                # Negative Feedback
                feedback_text = (
                    f"<b>Bad/Negative Result:</b><br>"
                    f"Your carbon footprint is above the {avg_type}, but small changes can help!<br>"
                    f"Try adopting sustainable practices in energy, waste, and travel to lower your impact.<br><br>"
                    f"<b>Energy:</b><br>"
                    f"- Switch to energy-efficient appliances and consider renewable options like solar panels.<br>"
                    f"- Improve home insulation to cut down on heating and cooling needs.<br>"
                    f"- Unplug devices when not in use to save energy.<br><br>"
                    f"<b>Waste:</b><br>"
                    f"- Try composting organic waste and using reusable bags to reduce single-use plastics.<br>"
                    f"- Join a recycling program to ensure proper waste processing.<br>"
                    f"- Donate items you no longer need instead of discarding them.<br><br>"
                    f"<b>Travel:</b><br>"
                    f"- Carpool or use public transport whenever possible.<br>"
                    f"- Walk or bike short distances to reduce emissions.<br>"
                    f"- Opt for virtual meetings to minimize travel when you can."
                )
    ...
    def fetch_europe_avg_and_update_table(self, business_type, year):
        try:
            ...
            # Query the database for Europe_Avg_CF and Europe_Industry_Avg_CF
            query = "SELECT Europe_AvgCF_KgCO2, Europe_Industry_AvgCF_KgCO2 FROM eu_avgcf_table WHERE Year=%s"
            mycursor.execute(query, (year,))
            result = mycursor.fetchone()

            if result:
                europe_avg_cf = result[0]
                europe_industry_avg_cf = result[1]

                # Calculate Industry Avg CO2
                industry_avg_co2 = europe_avg_cf / europe_industry_avg_cf if europe_industry_avg_cf != 0 else 0
 
                # Update carbonCalculator with the fetched values
                self.carbonCalculator["Details"].update({
                    "Europe_Avg_CF": europe_avg_cf,
                    "Europe_Industry_Avg_CF": europe_industry_avg_cf,
                ...
                self.table.setItem(4, 1, QTableWidgetItem("%.2f" % industry_avg_co2))
                self.table.item(4, 1).setTextAlignment(QtCore.Qt.AlignCenter | QtCore.Qt.AlignVCenter)

            else:
                QMessageBox.warning(self, "Data Not Found", f"No data found for year {year} in the database.")
                print(f"No data found for year: {year}")

        except Exception as e:
            QMessageBox.critical(self, "Error", f"Error fetching Europe averages: {e}")
            print(f"Error: {e}")
        finally:
            mycursor.close()
            mydb.close()
    ...
    def visualization(self, values:list):
        try:
            ...
            # Create a bar plot
            # Decide categories based on business type
            if self.sbusiness_rbtn.isChecked() or self.bbusiness_rbtn.isChecked():
                categories = ["Per Capita CF\n(KgCO2)", "Industry Avg CF\n(KgCO2)"]
                industry_avg_cf = self.carbonCalculator["Details"].get("Industry_Avg_CO2", 0)
                values = [per_capita_cf, industry_avg_cf]
            else:
